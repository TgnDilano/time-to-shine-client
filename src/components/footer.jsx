...
export default function Footer() {
  return (
    <footer className="w-screen py-4 padx bg-black items-center justify-center gap-y-6 text-white border-t-[2.5px] border-gold">
      <div className="flex justify-between text-zinc-400 my-4 items-center">
        <div className="flex-col w-[650px]">
          ...
          <p className="w-full mb-3">
            Toutes les photos doivent être prises dans le contexte de chaque
            sous thème à l’aide d'un Smartphone. Les retouches sont autorisées
            ainsi que les filtres. Tous les candidats ne respectant pas les sous
            thèmes énumérés <a className="text-white">dans cette page</a> seront
            éliminés.
          </p>
          <p className="w-full">
            Pour passer à la seconde étape, chaque candidat devra cumuler un
            maximum de points et seront retenus ceux qui auront bien évidement
            le plus de votes. Chaque vote vous sera débité d’un montant de 10
            FCFA chacun et susceptible d’évoluer au cours des étapes suivantes.
          </p>
        </div>
        <div>
        ...
        <div className="">
          ...
          <ul className="flex items-center flex-col gap-y-1">
            {Object.values(appRoutes).map((e) => (
              <li
                key={e.name}
                className="rounded bg-opacity-20 cursor-pointer hover:text-white duration-200 "
              >
      ...
      <div className="flex justify-between text-zinc-400 my-2 mt-2">
        <div className="flex gap-x-4 ">
          <p className="cursor-pointer uppercase hover:text-white">
            Privacy policy
          </p>
